...
def biquad_response(coeffs, freq_hz, fs=48000):
    ...
    mag2 = (
        (num_re**2 + num_im**2) / (den_re**2 + den_im**2)
        if (den_re**2 + den_im**2) != 0
        else 1.0
    )
...
class EQGraph(QWidget):
    ...
    def paintEvent(self, event):
        ...
        def get_eq_db(freq):
            return self.preamp + sum(
                biquad_response(c, freq) for c in getattr(self, "cached_coeffs", [])
            )
        ...
        if self.measurement_curve:
            for f, db in self.measurement_curve:
                ...
                if self.visibility.get("raw", True):
                    max_val = max(max_val, abs(db))
                if self.visibility.get("eq_meas", True):
                    max_val = max(max_val, abs(eq_db))

        if self.target_curve and self.visibility.get("target", True):
            for _, db in self.target_curve:
                max_val = max(max_val, abs(db))
        ...
        gain_steps = list(
            range(int(self.max_db_scale), int(-self.max_db_scale) - 1, -6)
        )
        for db in gain_steps:
            ...
            painter.setPen(
                QPen(
                    QColor(
                        ...
                        base_color.green(),
                        base_color.blue(),
                        opacity_low if db != 0 else opacity_high,
                    ),
                    1,
                )
            )
            ...
            if db != 0:
                painter.setPen(
                    QPen(
                        QColor(
                            base_color.red(), base_color.green(), base_color.blue(), 100
                        )
                    )
                )
        ...
        if self.target_curve and self.visibility.get("target", True):
            ...
            painter.setPen(
                QPen(
                    QColor(base_color.red(), base_color.green(), base_color.blue(), 80),
                    2,
                    Qt.DashLine,
                )
            )
        ...
        if self.visibility.get("eq", True):
            ...
            for i, f in enumerate(self.filters):
                ...
                painter.setPen(
                    QPen(
                        QColor("#FFFFFF" if i == self.active_idx else "#888888"),
                        2 if i == self.active_idx else 1,
                    )
                )
                painter.setBrush(
                    QBrush(color if i == self.active_idx else QColor("#444444"))
                )
        ...
        def draw_legend(key, text, color, is_dashed=False):
            ...
            painter.setPen(
                QPen(draw_color, 2, Qt.DashLine if is_dashed else Qt.SolidLine)
            )
            painter.drawLine(legend_x, legend_y - 4, legend_x + 20, legend_y - 4)

            text_color = (
                QColor(base_color.red(), base_color.green(), base_color.blue(), 200)
                if is_visible
                else QColor(100, 100, 100, 150)
            )
        ...
        if self.filters or self.preamp != 0:
        ...
        if self.visibility.get("eq", True):
            draw_legend(
                "ceiling", "Digital Ceiling", QColor(255, 165, 0, 200), is_dashed=True
            )
...
class FluentDACController(FluentWindow):
    ...
    def _delete_preset(self):
        ...
        if name == "None":
            InfoBar.warning(
                "Forbidden", "The 'None' preset cannot be deleted.", parent=self
            )
            return
        ...
        for idx, preset in enumerate(self.settings_data["presets"]):
            ...
            for i in range(10):
                ...
                if abs(pg) > 0.1 or abs(sg) > 0.1:
                    if (
                        abs(float(pf.get("freq", 1000)) - float(sf.get("freq", 1000)))
                        > 1.0
                    ):
                        match = False
                        break
    ...
    def load_settings(self):
        ...
        default_data = {
            "balance": 0,
            "last_preset": 0,
            "presets": [{"name": "None", "preamp": 0.0, "filters": default_filters}],
        }
            ...
            try:
                with open(SETTINGS_FILE, "r") as f:
                    ...
                    if not any(p["name"] == "None" for p in data.get("presets", [])):
                        data["presets"].insert(
                            0,
                            {"name": "None", "preamp": 0.0, "filters": default_filters},
                        )
    ...
    def _setup_ui(self):
        ...
        info_l.addWidget(self.lbl_man, 0, 0)
        info_l.addWidget(self.lbl_prod, 0, 1)
        info_l.addWidget(self.lbl_fw, 1, 0)
        info_l.addWidget(self.lbl_sn, 1, 1)
        ...
        mic_header.addStretch(1)
        mic_header.addWidget(self.mic_txt)
        ...
        vol_header.addWidget(self.vol_txt)
        ...
        self.vol_warning_lbl = CaptionLabel(
            "⚠️ Clipping Risk: DAC volume exceeds safe EQ headroom!", audio_card
        )
        self.vol_warning_lbl.setStyleSheet("color: #ff4d4d; font-weight: bold;")
        self.vol_warning_lbl.hide()
        audio_l.addWidget(self.vol_warning_lbl)
        audio_l.addSpacing(15)
        ...
        self.cb_filter, self.desc_filter = self._create_row(
            dac_layout,
            "Digital Filter",
            FILTER_MAP,
            0x11,
            "Selects the internal DAC reconstruction filter algorithm.",
        )
        self.cb_gain, self.desc_gain = self._create_row(
            dac_layout,
            "Gain Mode",
            GAIN_MAP,
            0x19,
            "Adjusts the amplifier's base volume scaling for sensitive IEMs or demanding headphones.",
        )
        self.cb_amp, self.desc_amp = self._create_row(
            dac_layout,
            "Amp Topology",
            AMP_MAP,
            0x1D,
            "Switches between Class AB (Maximum audio performance) and Class H (Higher power efficiency).",
        )

        self.cb_filter.currentTextChanged.connect(
            lambda t: self.desc_filter.setText(
                self.filter_desc_map.get(t, "Select a filter.")
            )
        )
        ...
        reset_desc = CaptionLabel(
            "Restore all device settings to factory defaults.", reset_card
        )
        ...
        io_bar = QHBoxLayout()
        self.btn_import_meas = PushButton(
            FluentIcon.MICROPHONE, "Import Measurement", header_frame
        )
        self.btn_import_meas.clicked.connect(self._import_measurement)
        io_bar.addWidget(self.btn_import_meas)

        self.btn_import_target = PushButton(
            FluentIcon.PIN, "Import Target Curve", header_frame
        )
        ...
        graph_layout.setContentsMargins(15, 0, 15, 20)
        ...
        self.active_band_card.setStyleSheet(
            "SimpleCardWidget { background: transparent; }"
        )
        bl = QHBoxLayout(self.active_band_card)
        bl.setContentsMargins(10, 5, 10, 5)

        self.band_lbl = StrongBodyLabel("Band 1", self.active_band_card)
        bl.addWidget(self.band_lbl)
        self.band_enable = CheckBox(self.active_band_card)
        self.band_enable.setFixedWidth(30)
        bl.addWidget(self.band_enable)

        self.band_type = ComboBox(self.active_band_card)
        self.band_type.addItems(["PK", "LS", "HS"])
        self.band_type.setFixedWidth(80)
        bl.addWidget(self.band_type)
        self.band_freq = LineEdit(self.active_band_card)
        self.band_freq.setFixedWidth(65)
        bl.addWidget(self.band_freq)
        bl.addWidget(CaptionLabel("Hz", self.active_band_card))
        self.lock_freq = CheckBox("Lock", self.active_band_card)
        bl.addWidget(self.lock_freq)

        self.band_gain_sld = Slider(Qt.Horizontal, self.active_band_card)
        self.band_gain_sld.setRange(-100, 100)
        bl.addWidget(self.band_gain_sld, 1)
        self.band_gain_val = LineEdit(self.active_band_card)
        self.band_gain_val.setFixedWidth(60)
        bl.addWidget(self.band_gain_val)
        bl.addWidget(CaptionLabel("dB", self.active_band_card))
        self.lock_gain = CheckBox("Lock", self.active_band_card)
        bl.addWidget(self.lock_gain)

        self.band_q_val = LineEdit(self.active_band_card)
        self.band_q_val.setFixedWidth(45)
        bl.addWidget(CaptionLabel("Q:", self.active_band_card))
        bl.addWidget(self.band_q_val)
        ...
        list_page_layout.setContentsMargins(15, 10, 15, 10)
        ...
        scroll = SmoothScrollArea(self.list_page)
        ...
        for i in range(10):
            ...
            band_card.setStyleSheet(
                "SimpleCardWidget { background: transparent; border: none; border-bottom: 1px solid rgba(128, 128, 128, 0.2); }"
            )
            ...
            chk = CheckBox(band_card)
            chk.setFixedWidth(30)
            typ = ComboBox(band_card)
            typ.addItems(["PK", "LS", "HS"])
            typ.setFixedWidth(80)
            freq = LineEdit(band_card)
            freq.setFixedWidth(65)

            lbl_layout.addWidget(chk)
            lbl_layout.addWidget(typ)
            lbl_layout.addWidget(freq)
            lbl_layout.addWidget(CaptionLabel("Hz", band_card))
            ...
            lbl_layout.addWidget(gain)
            lbl_layout.addWidget(CaptionLabel("dB", band_card))

            qv = LineEdit(band_card)
            qv.setFixedWidth(45)
            lbl_layout.addSpacing(15)
            lbl_layout.addWidget(CaptionLabel("Q:", band_card))
            lbl_layout.addWidget(qv)

            chk.stateChanged.connect(lambda _, idx=i: self._on_list_ui_changed(idx))
            typ.currentIndexChanged.connect(
                lambda _, idx=i: self._on_list_ui_changed(idx)
            )
            sld.valueChanged.connect(
                lambda v, idx=i, le=gain: [
                    le.setText(str(v / 10)),
                    self._on_list_ui_changed(idx),
                ]
            )
            gain.editingFinished.connect(
                lambda s=sld, le=gain: s.setValue(
                    int(float(le.text().replace(",", ".") or 0) * 10)
                )
            )
        ...
        list_page_layout.addWidget(scroll)
        ...
        card_layout.addWidget(
            SubtitleLabel("TRN Control Panel", about_card), 0, Qt.AlignCenter
        )
    ...
    def _toggle_sn(self, event):
        if not self.actual_sn:
            return
        self.sn_hidden = not self.sn_hidden
        self.lbl_sn.setText(
            f"Serial: {'********' if self.sn_hidden else self.actual_sn}"
        )
    ...
    def _check_headroom(self, auto_level=False):
        ...
        if active_coeffs:
            ...
            for f_hz in freqs_to_check:
                max_db = max(
                    max_db, sum(biquad_response(c, f_hz) for c in active_coeffs)
                )
        ...
        pct = max(
            ...
            min(
                100,
                int(
                    ((self.last_raw_vol - VOL_MIN_RAW) / (VOL_MAX_RAW - VOL_MIN_RAW))
                    * 100
                ),
            ),
        )
        ...
        self.vol_txt.setText(f"{pct}%")
        self.eq_vol_slider.setValue(pct)
        self.eq_vol_txt.setText(f"{pct}%")
        self._updating_ui = False

    def _on_volume_slider_changed(self, pos):
        if self._updating_ui or self.is_syncing:
            return
        self.last_raw_vol = int(
            VOL_MIN_RAW + (pos / 100.0) * (VOL_MAX_RAW - VOL_MIN_RAW)
        )
        self._check_headroom(auto_level=False)
        self._apply_filter(-1)
    ...
    def _factory_reset(self):
        ...
        InfoBar.success(
            "Factory Reset",
            "All settings, including Mic Gain, restored to defaults.",
            parent=self,
        )
    ...
    def _on_visual_ui_changed(self, *_):
        ...
        try:
            f["freq"] = max(20, min(20000, int(self.band_freq.text())))
        except:
            pass
        ...
        try:
            f["q"] = max(
                0.1, min(18.0, float(self.band_q_val.text().replace(",", ".")))
            )
        except:
            pass
    ...
    def _on_list_ui_changed(self, idx):
        ...
        try:
            f["freq"] = max(20, min(20000, int(lw["freq"].text())))
        except:
            pass
        f["gain"] = lw["slider"].value() / 10.0
        try:
            f["q"] = max(0.1, min(18.0, float(lw["q"].text())))
        except:
            pass
    ...
    def _import_rew_target(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Open REW Target", "", "Text/CSV Files (*.txt *.csv);;All Files (*)"
        )
        if not path:
            return
        try:
            ...
            if normalized:
                ...
                InfoBar.success(
                    "Success",
                    f"Loaded target curve ({len(normalized)} points)",
                    parent=self,
                )
            else:
                InfoBar.warning(
                    "Warning", "No valid frequency/dB data found.", parent=self
                )
    ...
    def _import_measurement(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Open Measurement", "", "Text/CSV Files (*.txt *.csv);;All Files (*)"
        )
        if not path:
            return
        try:
            ...
            if normalized:
                ...
                InfoBar.success(
                    "Success",
                    f"Loaded measurement curve ({len(normalized)} points)",
                    parent=self,
                )
            else:
                InfoBar.warning(
    ...
    def _check_connection(self):
        ...
        try:
            attached = any(
                d["vendor_id"] == VID and d["product_id"] == PID
        ...
        if not attached:
            ...
            if not self.active_device:
                # Reset syncing flag before attempting refresh after reconnect
    ...
    def toggle_controls(self, enabled):
        objs = [
            ...
            self.btn_add,
            self.btn_delete,
            self.btn_import_meas,
            self.btn_import_target,
            self.btn_import,
            self.btn_export,
            self.graph,
            self.small_graph,
            self.btn_toggle_view,
            self.active_band_card,
            self.band_enable,
            self.band_type,
            self.band_freq,
            self.band_gain_sld,
    ...
    def _create_row(self, layout, title, mapping, cmd, default_desc=""):
        ...
        cb = ComboBox(card)
        cb.addItems(list(mapping.values()))
        cb.setFixedWidth(220)
        cb.currentIndexChanged.connect(
            lambda: self.write_val(cmd, cb.currentText(), mapping)
        )
            ...
            try:
                ...
                def send_and_wait(pkt, key, timeout=0.15, is_peq=False):
                    ...
                    while time.time() - start < timeout:
                        if is_peq and key in self.parsed_filters:
                            return True
                        if not is_peq and key in self.read_results:
                            return True
                        time.sleep(0.01)
                    return False

                def pad(p):
                    return p + [0x00] * (65 - len(p))
                ...
                for pkt, key in tasks:
                    for _ in range(2):
                        if send_and_wait(pkt, key):
                            break

                for i in range(10):
                    pkt = pad([REPORT_ID, READ, CMD_PEQ_VALUES, 0x00, 0x00, i, END])
                    for _ in range(2):
                        if send_and_wait(pkt, i, is_peq=True):
                            break

                with self.usb_lock:
                    sn = dev.get_serial_number_string() or "Unknown"

                self.comm.sync_finished.emit(
                    {"SN": sn}, self.read_results, self.parsed_filters
                )
            except Exception as e:
    ...
    def on_data(self, data):
        if data[0] == REPORT_ID and data[1] == READ:
            ...
            if cmd == CMD_VERSION:
                ...
                self.parsed_filters[idx] = {
                    "freq": f,
                    "q": q,
                    "gain": g,
                    "type": INV_TYPE_CODES.get(data[34], "PK"),
                }
                ...
                if self.is_syncing:
                    self.last_raw_vol = raw_vol
                elif raw_vol != getattr(self, "last_raw_vol", 0) and not getattr(
                    self, "_updating_ui", False
                ):
    ...
    def update_ui_state(self, info, results, filters):
        ...
        self.bal_txt.setText(
            f"L {abs(bal)}" if bal < 0 else f"R {bal}" if bal > 0 else "Center"
        )
        ...
        mapping = {
            0x11: (self.cb_filter, FILTER_MAP),
            0x19: (self.cb_gain, GAIN_MAP),
            0x1D: (self.cb_amp, AMP_MAP),
        }
        for cmd, (w, m) in mapping.items():
            if cmd in results:
                w.setCurrentText(m.get(results[cmd], "Unknown"))
        self.desc_filter.setText(
            self.filter_desc_map.get(self.cb_filter.currentText(), "")
        )
        ...
        if matched_idx != -1:
            ...
            none_idx = next(
                (
                    i
                    for i, p in enumerate(self.settings_data["presets"])
                    if p["name"] == "None"
                ),
                0,
            )
            ...
            try:
                ...
                for idx in tasks:
                    if idx >= 0:
                        ...
                        g = (
                            0.0
                            if not f_data.get("enabled", True)
                            else float(f_data["gain"])
                        )
                        f, q, t = (
                            max(1, int(f_data["freq"])),
                            max(0.01, float(f_data["q"])),
                            f_data["type"],
                        )
                        A, w0 = 10 ** (g / 40), 2 * math.pi * f / 48000
                        ...
                        if t == "PK":
                            ...
                            sqA, s = math.sqrt(A), 1 if t == "HS" else -1
                            b0, b1, b2 = (
                                A * ((A + 1) + s * (A - 1) * cs + 2 * sqA * alpha),
                                -s * 2 * A * ((A - 1) + s * (A + 1) * cs),
                                A * ((A + 1) + s * (A - 1) * cs - 2 * sqA * alpha),
                            )
                            a0, a1, a2 = (
                                (A + 1) - s * (A - 1) * cs + 2 * sqA * alpha,
                                s * 2 * ((A - 1) - s * (A + 1) * cs),
                                (A + 1) - s * (A - 1) * cs - 2 * sqA * alpha,
                            )
                        else:
                        ...
                        hw_biquads = b"".join(
                            struct.pack("<f", c / a0) for c in [b0, b1, b2, a1, a2]
                        )
                        pkt = [
                            WRITE,
                            CMD_PEQ_VALUES,
                        ...
                        pkt += (
                            list(struct.pack("<H", f))
                            + list(struct.pack("<H", int(q * 256)))
                            + list(struct.pack("<h", int(g * 256)))
                            + [
                                TYPE_CODES.get(t, 0x02),
                                0x00,
                                getattr(self, "active_slot", 0x00),
                                END,
                            ]
                        )
            ...
            with self.usb_lock:
                ...
                if dev:
                    try:
                        # Pad to EXACTLY 65 bytes (1 ID + 64 Data)
                        dev.write(
                            [REPORT_ID, WRITE, CMD_FLASH_EQ, 0x01, END] + [0x00] * 60
                        )
                    except:
                        pass
            time.sleep(0.2)
    ...
    def _on_balance_change(self):
        ...
        self.bal_txt.setText(
            f"L {abs(v)}" if v < 0 else f"R {v}" if v > 0 else "Center"
        )
        if self.is_syncing:
            return
        ...
        with self.usb_lock:
            ...
            if dev:
                try:
                    ...
                    dev.write(
                        [REPORT_ID, 0x01, 0x16, 0x04, 0x01, 0x00, mag_l] + [0x00] * 58
                    )
                    time.sleep(0.01)
                    dev.write(
                        [REPORT_ID, 0x01, 0x16, 0x04, 0x00, 0x00, mag_r] + [0x00] * 58
                    )
    ...
    def _new_preset(self):
        dialog = CustomInputDialog(
            "Create New Preset", "Enter a unique name for your profile:", self
        )
        ...
        if dialog.exec():
            ...
            if name:
                ...
                self.settings_data["presets"].append(
                    {"name": name, "preamp": 0.0, "filters": df}
                )
                ...
                InfoBar.success(
                    "Preset Created",
                    f"'{name}' is ready to be configured.",
                    parent=self,
                )

    def _import_squig(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Open AutoEQ File", "", "Text Files (*.txt);;All Files (*)"
        )
        if not path:
            return
        try:
            ...
            for line in lines:
                ...
                if "Filter" in line and idx < 10:
                    ...
                    f_data["type"] = (
                        "PK" if " PK " in line else "LS" if " LS " in line else "HS"
                    )
                    fc, gn, qv = (
                        re.search(r"Fc\s+([\d.]+)", line),
                        re.search(r"Gain\s+([-+.\d]+)", line),
                        re.search(r"Q\s+([\d.]+)", line),
                    )
                    if fc:
                        f_data["freq"] = max(20, min(20000, float(fc.group(1))))
                    if gn:
                        f_data["gain"] = max(-10.0, min(10.0, float(gn.group(1))))
                    if qv:
                        f_data["q"] = max(0.1, float(qv.group(1)))
                    idx += 1
            for i in range(10):
    ...
    def _export_autoeq(self):
        ...
        try:
            ...
            for i, f in enumerate(p["filters"]):
                ...
                q = f.get("q", 1.0)
                lines.append(
                    f"Filter {i + 1}: {state} {t} Fc {fc:.1f} Hz Gain {g:.1f} dB Q {q:.2f}\n"
                )
            ...
            InfoBar.success(
                "Success", f"Preset exported to {os.path.basename(path)}", parent=self
            )
        except Exception as e:
    ...
    def write_val(self, cmd, selection, n_map):
        ...
        with self.usb_lock:
            ...
            if dev:
                try:
                    dev.write(
                        [REPORT_ID, WRITE, cmd, 0x01, inv.get(selection)] + [0x00] * 59
                    )
                except:
                    pass
